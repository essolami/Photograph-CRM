...
export async function createUser(_state: UserActionState, data: FormData): Promise<UserActionState> {
  await requirePermission("canManageUsers");
  const name = String(data.get("name") ?? "").trim();
  const email = String(data.get("email") ?? "").trim().toLowerCase();
  const password = String(data.get("password") ?? "");
  if (!name || !/^\S+@\S+\.\S+$/.test(email)) return { error: "Enter a name and valid email." };
  if (password.length < 8) return { error: "Password must be at least 8 characters." };
  try {
    await prisma.user.create({ data: { name, email, passwordHash: await hashPassword(password), isActive: true, canViewClients: checked(data, "canViewClients"), canManageClients: checked(data, "canManageClients"), canViewInvoices: checked(data, "canViewInvoices"), canManageInvoices: checked(data, "canManageInvoices"), canManageUsers: checked(data, "canManageUsers") } });
